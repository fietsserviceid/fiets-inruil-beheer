...
// ===== Codes (Phase 3) + Option A integration =====
let GH={owner:'fietsserviceid',repo:'fiets-inruil-calculator',path:'codes.json',branch:'main'}; 
let CODES_FULL=null; let CODES_SHA=null; let CODES_VIEW=[];
let DEALERS_LIST=[];

async function ensureDealersLoaded(){ if(DEALERS_LIST.length) return; try{ const r=await fetch('./dealers.json'); DEALERS_LIST=await r.json(); }catch(e){ console.error('Dealers laden mislukt',e); }}

function renderDealerDatalist(){ const dl=document.getElementById('dealerNames'); if(!dl) return; if(dl.dataset.ready==='1') return; dl.innerHTML = (DEALERS_LIST||[]).filter(d=>d.active!==false).map(d=>`<option value="${escapeHtml(d.name)}"></option>`).join(''); dl.dataset.ready='1'; }
...
async function loadCodesFromGitHub(){ try{ setCodesStatus('Laden...'); const url=`https://api.github.com/repos/${GH.owner}/${GH.repo}/contents/${GH.path}?ref=${GH.branch}`; const res=await fetch(url,{headers:ghHeaders()}); if(!res.ok){const txt=await res.text(); throw new Error('GitHub GET fout: '+res.status+' '+txt);} const data=await res.json(); CODES_SHA=data.sha; const content=atob(data.content.replace(/\n/g,'')); CODES_FULL=JSON.parse(content); CODES_VIEW=[...CODES_FULL.codes]; renderCodes(CODES_VIEW); document.getElementById('codes-meta').textContent=`version: ${CODES_FULL.version} • price_eur_per_year: €${CODES_FULL.price_eur_per_year}`; setCodesStatus('Gereed.'); }catch(e){ console.error(e); setCodesStatus('Kon codes.json niet laden: '+e.message); }}

function renderCodes(list){ renderDealerDatalist(); const tbody=document.querySelector('#codesTable tbody'); tbody.innerHTML=''; list.forEach((c)=>{ const exp=c.expires? String(c.expires).slice(0,10):''; const safeNote=escapeHtml(c.note||''); tbody.insertAdjacentHTML('beforeend', `
<tr>
  ...
  tbody.querySelectorAll('input').forEach(inp=>{ inp.addEventListener('change', onCodeFieldChange); inp.addEventListener('input', (e)=>{ if(e.target.type==='date'||e.target.type==='text') onCodeFieldChange(e); }); }); }

function escapeHtml(s){return s?String(s).replace(/[&<>"']/g, m=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;','\'':'&#39;'}[m])):'';}

function isValidDealerName(name){ const n=String(name||'').toLowerCase(); return (DEALERS_LIST||[]).some(d=> (d.active!==false) && String(d.name||'').toLowerCase()===n ); }

function onCodeFieldChange(e){ const field=e.target.getAttribute('data-field'); const code=e.target.getAttribute('data-code'); const obj=CODES_FULL?.codes?.find(x=>x.code===code); if(!obj) return; if(field==='active'){ obj.active=e.target.checked; } else if(field==='note'){ const val=e.target.value; if(!isValidDealerName(val)){ e.target.setCustomValidity('Kies een dealer uit de lijst'); e.target.reportValidity(); return; } e.target.setCustomValidity(''); obj.note=val; } else if(field==='expires'){ obj.expires=e.target.value? e.target.value:null; } }
...
async function saveCodesToGitHub(){ if(!CODES_FULL){alert('Nog geen codes geladen.');return;} const t=sessionStorage.getItem('ghToken'); if(!t){alert('Voer eerst je GitHub token in bij Instellingen.'); return;} try{ setCodesStatus('Opslaan bezig...'); const newObj={ price_eur_per_year: CODES_FULL.price_eur_per_year, version: CODES_FULL.version+1, codes: CODES_FULL.codes }; const jsonStr=JSON.stringify(newObj, null, 2)+'\n'; const b64=btoa(unescape(encodeURIComponent(jsonStr))); const url=`https://api.github.com/repos/${GH.owner}/${GH.repo}/contents/${GH.path}`; const body={ message:'FSID beheer: update codes.json (active/expires/note from dealers list)', content:b64, sha:CODES_SHA, branch:GH.branch }; const res=await fetch(url,{method:'PUT', headers:{...ghHeaders(),'Content-Type':'application/json'}, body:JSON.stringify(body)}); if(!res.ok){const txt=await res.text(); throw new Error('GitHub PUT fout: '+res.status+' '+txt);} const result=await res.json(); CODES_SHA=result.content.sha; CODES_FULL.version=newObj.version; document.getElementById('codes-meta').textContent=`version: ${CODES_FULL.version} • price_eur_per_year: €${CODES_FULL.price_eur_per_year}`; setCodesStatus('Opgeslagen ✔'); }catch(e){ console.error(e); setCodesStatus('Opslaan mislukt: '+e.message); } }
