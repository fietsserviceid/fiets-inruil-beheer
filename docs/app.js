...
// ===== CODES (Phase 3) =====
let GH={owner:'fietsserviceid',repo:'fiets-inruil-calculator',path:'codes.json',branch:'main'}; // defaults
let CODES_FULL=null; // {price_eur_per_year, version, codes:[]}
let CODES_SHA=null;  // Git blob sha required for updates
let CODES_VIEW=[];   // filtered view
...
async function saveCodesToGitHub(){
  ...
  try{
    ...
    // Bouw nieuwe content (behoud version en price)
    ...
    CODES_SHA=result.content.sha; // update local sha
    CODES_FULL.version=newObj.version; // reflect bumped version
    document.getElementById('codes-meta').textContent=`version: ${CODES_FULL.version} • price_eur_per_year: €${CODES_FULL.price_eur_per_year}`;
