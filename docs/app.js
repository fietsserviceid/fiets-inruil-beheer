...
function filterDealers(){ 
 ...
 renderDealers( 
 DEALERS.filter(d=> (String(d.name||'').toLowerCase().includes(q) 
...
// NEW: explicit reload button handler
...
function onCodeFieldChange(e){ 
 ...
 const obj=CODES_FULL?.codes?.find(x=>x.code===code); if(!obj) return; 
 if(field==='active'){ obj.active = e.target.checked; } 
 else if(field==='note'){ 
 const val=e.target.value; const n=String(val||'').toLowerCase(); 
 ...
 e.target.setCustomValidity(''); obj.note = val; 
 } else if(field==='expires'){ obj.expires = e.target.value ? e.target.value : null; } 
} 
...
async function saveCodesToGitHub(){ 
 ...
 const t=localStorage.getItem('ghToken'); if(!t){ alert('Voer eerst je GitHub token in bij Instellingen.'); return; } 
 try{ 
