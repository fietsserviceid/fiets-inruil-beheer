...
function base32ToBytes(base32){
  ...
  let bits='';
  for(const c of base32){ const i=alphabet.indexOf(c); if(i<0) continue; bits+=i.toString(2).padStart(5,'0'); }
  const out=[]; for(let i=0;i+8<=bits.length;i+=8) out.push(parseInt(bits.substring(i,i+8),2));
...
async function generateTOTP(secret){
  ...
  const bytes=new Uint8Array(hmac); const offset=bytes[19]&0xf;
...
function logout(){ localStorage.removeItem('ghToken'); sessionStorage.clear(); location.reload(); }
...
function saveSettings(){
  ...
  const t=document.getElementById('ghToken').value; if(t) localStorage.setItem('ghToken',t);
  closeSettings();
}
function ghHeaders(){ const h={'Accept':'application/vnd.github+json'}; const t=localStorage.getItem('ghToken'); if(t) h['Authorization']='Bearer '+t; return h; }
...
async function loadDealersFromGitHub(){
  try{
    ...
    DEALERS_SHA=data.sha; const content=atob(data.content.replace(/\n/g,''));
...
function filterDealers(){ const q=document.getElementById('dealerSearch').value.toLowerCase(); renderDealers(DEALERS.filter(d=> (String(d.name||'').toLowerCase().includes(q)||String(d.city||'').toLowerCase().includes(q)||String(d.code||'').toLowerCase().includes(q)))); }
function nextDealerCode(){ let max=0; const re=/^DLR-(\d{4})$/i; for(const d of DEALERS){ const m=String(d.code||'').match(re); if(m) max=Math.max(max, parseInt(m[1],10)); } const n=String(max+1).padStart(4,'0'); return `DLR-${n}`; }
...
function deleteDealer(id){ if(!confirm('Weet je zeker dat je deze dealer wilt verwijderen?')) return; DEALERS=DEALERS.filter(d=>d.id!==id); renderDealers(DEALERS); saveDealersToGitHub('delete'); }
async function saveDealersToGitHub(action){
  if(!DEALERS){ alert('Nog geen dealers geladen.'); return; }
  const t=localStorage.getItem('ghToken'); if(!t){ alert('Voer eerst je GitHub token in bij Instellingen.'); try{openSettings();}catch(_){} return; }
  try{
    ...
    const result=await res.json(); DEALERS_SHA=result.content.sha; setDealersStatus('Opgeslagen ✔');
...
function saveDealer(){
  ...
  else{ const id='d'+Math.floor(Math.random()*1e9).toString(36); DEALERS.push({id,name,city,code,active}); action='create'; }
...
async function ensureDealersLoaded(force=false){
  ...
  try{
    ...
    if(res.ok){ const data=await res.json(); const content=atob(data.content.replace(/\n/g,'')); dealers=JSON.parse(content); }
    else{ const raw=`https://raw.githubusercontent.com/${GHD.owner}/${GHD.repo}/${GHD.branch}/${GHD.path}`; const r2=await fetch(raw); if(!r2.ok){ const txt=await res.text().catch(()=> ''); throw new Error('GET dealers (datalist): '+res.status+' '+txt); } dealers=await r2.json(); }
    DEALERS_LIST=Array.isArray(dealers)? dealers:[]; renderDealerDatalist();
  }catch(e){ console.warn('Dealers laden voor datalist mislukt (optioneel):',e); }
}
function renderDealerDatalist(){
  const dl=document.getElementById('dealerNames'); if(!dl) return;
  dl.innerHTML=(DEALERS_LIST||[]).filter(d=>d.active!==false).map(d=>`<option value="${escapeHtml(d.name)}"></option>`).join('');
}
async function reloadDealers(){ try{ setCodesStatus('Dealers herladen...'); await ensureDealersLoaded(true); renderDealerDatalist(); setCodesStatus('Dealers bijgewerkt ✔'); }catch(e){ console.error(e); setCodesStatus('Herladen dealers mislukt: '+e.message);} }
async function showCodes(){ document.getElementById('dealers-screen').style.display='none'; document.getElementById('codes-screen').style.display='block'; await ensureDealersLoaded(true); renderDealerDatalist(); loadCodesFromGitHub(); }
...
async function loadCodesFromGitHub(){ try{ setCodesStatus('Laden...'); const res=await fetch(ghCodesUrl(),{headers:ghHeaders()}); if(!res.ok){ const txt=await res.text(); throw new Error('GET codes: '+res.status+' '+txt); } const data=await res.json(); CODES_SHA=data.sha; const content=atob(data.content.replace(/\n/g,'')); CODES_FULL=JSON.parse(content); CODES_VIEW=[...CODES_FULL.codes]; renderDealerDatalist(); renderCodes(CODES_VIEW); document.getElementById('codes-meta').textContent=`version: ${CODES_FULL.version} • price_eur_per_year: €${CODES_FULL.price_eur_per_year}`; setCodesStatus('Gereed.'); }catch(e){ console.error(e); setCodesStatus('Kon codes.json niet laden: '+e.message); }}
function renderCodes(list){ const tbody=document.querySelector('#codesTable tbody'); tbody.innerHTML=''; list.forEach(c=>{ const exp=c.expires? String(c.expires).slice(0,10):''; const safeNote=escapeHtml(c.note||''); tbody.insertAdjacentHTML('beforeend',`<tr><td style="font-family:monospace;">${c.code}</td><td><input list="dealerNames" data-field="note" data-code="${c.code}" value="${safeNote}" style="width:95%"></td><td style="text-align:center"><input type="checkbox" data-field="active" data-code="${c.code}" ${c.active?'checked':''}></td><td><input type="date" data-field="expires" data-code="${c.code}" value="${exp}"></td></tr>`); }); tbody.querySelectorAll('input').forEach(inp=>{ inp.addEventListener('change', onCodeFieldChange); inp.addEventListener('input', e=>{ if(e.target.type==='date'||e.target.type==='text') onCodeFieldChange(e); }); }); }
function onCodeFieldChange(e){ const field=e.target.getAttribute('data-field'); const code=e.target.getAttribute('data-code'); const obj=CODES_FULL?.codes?.find(x=>x.code===code); if(!obj) return; if(field==='active'){ obj.active=e.target.checked; } else if(field==='note'){ const val=e.target.value; const n=String(val||'').toLowerCase(); const ok=(DEALERS_LIST||[]).some(d=> (d.active!==false) && String(d.name||'').toLowerCase()===n); if(!ok){ e.target.setCustomValidity('Kies een dealer uit de lijst'); e.target.reportValidity(); return; } e.target.setCustomValidity(''); obj.note=val; } else if(field==='expires'){ obj.expires=e.target.value? e.target.value:null; } }
function filterCodes(){ const q=document.getElementById('codesSearch').value.toLowerCase(); if(!CODES_FULL) return; CODES_VIEW=CODES_FULL.codes.filter(c=> String(c.code||'').toLowerCase().includes(q) || String(c.note||'').toLowerCase().includes(q)); renderCodes(CODES_VIEW); }
async function saveCodesToGitHub(){ if(!CODES_FULL){ alert('Nog geen codes geladen.'); return; } const t=localStorage.getItem('ghToken'); if(!t){ alert('Voer eerst je GitHub token in bij Instellingen.'); return; } try{ setCodesStatus('Opslaan...'); const newObj={ price_eur_per_year:CODES_FULL.price_eur_per_year, version:CODES_FULL.version+1, codes:CODES_FULL.codes }; const jsonStr=JSON.stringify(newObj,null,2)+'\n'; const b64=btoa(unescape(encodeURIComponent(jsonStr))); const url=`https://api.github.com/repos/${GH.owner}/${GH.repo}/contents/${GH.path}`; const body={ message:'FSID beheer: update codes.json (active/expires/note)', content:b64, sha:CODES_SHA, branch:GH.branch }; const res=await fetch(url,{method:'PUT',headers:{...ghHeaders(),'Content-Type':'application/json'},body:JSON.stringify(body)}); if(!res.ok){ const txt=await res.text(); throw new Error('PUT codes: '+res.status+' '+txt); } const result=await res.json(); CODES_SHA=result.content.sha; CODES_FULL.version=newObj.version; document.getElementById('codes-meta').textContent=`version: ${CODES_FULL.version} • price_eur_per_year: €${CODES_FULL.price_eur_per_year}`; setCodesStatus('Opgeslagen ✔'); }catch(e){ console.error(e); setCodesStatus('Opslaan mislukt: '+e.message); }}
